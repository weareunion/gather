...
use Union\API\exceptions\reporting\ErrorReport;
use Union\PKG\Autoloader;
include_once "/lib/union/lib/REST/v1/API/exceptions/classes/ErrorReport.php";
try{
    // Check if safe mode is not set
    if(!defined("UNION_SAFEMODE")){
        ErrorReport::listen();
        if (UNION_PREFERENCES_LISTEN_ALL) {
            ErrorReport::listen_all();
        }
    }
...
abstract class FormattedException extends \Exception implements IException
{
    ...
    public function __construct($internal_title, $internal_description = "None.", $external_title = null, $external_description = null, $no_external_facing_EC = false, $code = 0, $lookup_code=null)
    {
    ...
    {
        return "[". $this->data["log_lookup"] . "] " . get_class($this) . " '{$this->data['internal']['title']}' - '{$this->data['internal']['description']}' ('{$this->data['code']}') in {$this->file}({$this->line})\n"
...
// Global Catcher
class UncaughtException extends FormattedException {};


// API
class APIException extends FormattedException {};
class InvalidParams extends APIException {};
class InvalidParamType extends APIException {};
...
class CardException extends APIException {}
    ...
    class CardTransactionException extends CardException {}
        class InvalidCardTransactionType extends CardTransactionException {}
//API.accounts
